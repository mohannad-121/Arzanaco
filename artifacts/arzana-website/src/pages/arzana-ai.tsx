...
export default function ArzanaAi() {
  ...
  const initialMessage: Message = { role: 'assistant', content: language === 'ar' ? 'مرحبًا بك في مساعد أرزانا الذكي. يمكنني مساعدتك في استكشاف منتجات أرزانا العربية وخدماتها وأنظمة السلامة وخدمات الاختبار والتشغيل وطرق التواصل وطلب عرض السعر.' : 'Welcome to ARZANA AI. I can help you explore Arzana Arabia’s products, services, safety solutions, testing capabilities, contact information, and quotation process.' };
  const [messages, setMessages] = useState<Message[]>([initialMessage]);
  const [input, setInput] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');
  const copy = language === 'ar' ? { title: 'مساعد أرزانا الذكي', subtitle: 'إجابات مستندة إلى معلومات أرزانا العامة والمتحقق منها.', placeholder: 'اكتب سؤالك عن أرزانا...', send: 'إرسال', clear: 'بدء محادثة جديدة', suggested: 'أسئلة مقترحة', unavailable: 'مساعد أرزانا الذكي غير متاح مؤقتاً. يرجى التواصل مع أرزانا عبر واتساب أو البريد الإلكتروني.' } : { title: 'ARZANA AI', subtitle: 'Answers grounded in Arzana’s verified public company information.', placeholder: 'Ask a question about Arzana...', send: 'Send', clear: 'Start a new conversation', suggested: 'Suggested questions', unavailable: 'ARZANA AI is temporarily unavailable. Please contact Arzana through WhatsApp or email.' };
  ...
  const sendMessage = async (value = input) => {
    ...
    try {
      const response = await fetch('/api/arzana-ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: content, language, history: messages.slice(-8).map(({ role, content: historyContent }) => ({ role, content: historyContent })) }) });
