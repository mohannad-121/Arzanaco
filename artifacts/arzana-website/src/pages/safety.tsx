  ...
  return (
    <PageWrapper>
      <section className="page-hero py-28 text-background md:py-36">
        ...
        <div className="site-container relative z-10 max-w-4xl"><p className="eyebrow mb-5 !text-white/70">Protective systems</p>
      ...
      <section className="bg-background py-16 md:py-24"><div className="site-container">
          <div className="mx-auto grid max-w-5xl grid-cols-1 gap-6 md:grid-cols-2">
            {safetyProducts.map((product) => (
              <Link
                key={product.id}
                href={`/products/${category?.slug}/${product.slug}`}
                className="group border border-border bg-card p-7 transition-[color,background-color,transform] hover:-translate-y-1 hover:border-primary hover:bg-[#27282a] hover:text-white"
              >
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-primary">{copy.catalog}</p>
                <h2 className="mt-3 text-xl font-semibold text-foreground group-hover:text-white">
