  ...
  const copy =
    language === "ar"
      ? {
          ...
          introduction:
            "يسرد ملف الشركة اختبار وتشغيل المحطات الكهربائية والأنظمة التالية.",
      ...
      : {
          ...
          introduction:
            "The company profile lists testing and commissioning of electrical substations and the systems below.",
    ...
    <PageWrapper>
      ...
      <section className="bg-[#d8d4cb] py-16 md:py-24">
        <div className="site-container max-w-5xl">
          <p className="eyebrow mb-4">Commissioning scope</p>
          <h2 className="section-title mb-8 text-3xl">{copy.systems}</h2>
          <div className="steel-card p-6 md:p-8">
            <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {systemsCovered.map((system) => (
                <li
                  key={system}
                  className="flex items-start gap-3 border border-border bg-muted/70 p-4"
                >
                  <CheckCircle2
          ...
          <section
            ...
            aria-label={
              language === "ar"
                ? "معرض صور الاختبار والتشغيل"
                : "Testing and commissioning gallery"
            }
          >
            <CoverflowCarousel
              items={testingCommissioningMedia.map((item) => ({
                src: item.src,
                alt: language === "ar" ? item.altAr : item.altEn,
                caption: language === "ar" ? item.altAr : item.altEn,
              }))}
            />
          </section>
