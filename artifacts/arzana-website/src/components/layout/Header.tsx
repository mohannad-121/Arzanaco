...
export const Header = () => {
  ...
  const navClass = (href: string) => cn(
    'relative flex min-h-11 items-center px-2.5 text-[.72rem] font-bold uppercase tracking-[.11em] transition-colors after:absolute after:inset-x-2.5 after:bottom-1 after:h-px after:origin-start after:scale-x-0 after:transition-transform after:duration-200',
    isFloating ? 'text-white/80 hover:text-white after:bg-white' : 'text-foreground/72 hover:text-primary after:bg-primary',
    location === href && (isFloating ? 'text-white after:scale-x-100' : 'text-primary after:scale-x-100'),
  );
  ...
  return <header className={cn('fixed inset-x-0 top-0 z-50 h-[76px] transition-[background-color,border-color,box-shadow] duration-300 md:h-[84px]', isFloating ? 'border-b border-white/10 bg-gradient-to-b from-black/30 to-transparent' : 'border-b border-[#d5d1c9] bg-[#f7f4ef]/95 text-foreground shadow-[0_7px_26px_rgba(31,32,34,.09)] backdrop-blur-md')}>
    ...
    <div className="site-container flex h-full items-center justify-between gap-4">
      ...
      <nav className="hidden items-center gap-0.5 xl:flex" aria-label="Primary navigation">
        ...
        <div className="group relative"><Link href="/products" className={cn(navClass('/products'), location.startsWith('/products') && (isFloating ? 'text-white after:scale-x-100' : 'text-primary after:scale-x-100'))}>{t('nav.products')}<ChevronDown className="ms-1 h-3.5 w-3.5 transition-transform duration-200 group-hover:rotate-180" /></Link>
      ...
      <div className="hidden items-center gap-3 xl:flex"><button type="button" onClick={() => setLanguage(language === 'en' ? 'ar' : 'en')} className={cn('min-h-10 border-s px-3 text-xs font-bold tracking-[.14em] transition-colors', isFloating ? 'border-white/25 text-white hover:text-white/65' : 'border-foreground/15 text-foreground hover:text-primary')} aria-label="Switch language">{language === 'en' ? 'AR' : 'EN'}</button><Button onClick={() => setLocation('/request-quote')} className="min-h-11 rounded-sm border border-primary/80 px-4 shadow-none hover:shadow-none">{t('nav.quote')}<ArrowUpRight className="ms-1 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></Button></div>
